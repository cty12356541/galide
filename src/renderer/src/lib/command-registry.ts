/**
 * command-registry — 命令注册表 + 快捷键解析(P0:打通录制链路)
 *
 * 设计:
 *   - 所有可绑快捷键的命令在此登记,带默认 accelerator(单一真相源)
 ...
 *
 * 不在此登记的命令(仅菜单/按钮,无快捷键)如"AI 浮出"不纳入,避免假条目。
 */
...
/** 命令 → 默认 accelerator(单一真相源,偏好面板与键盘 hook 共用) */
export const COMMANDS: readonly CommandDef[] = [
  { id: 'commandPalette', label: '命令面板', default: 'Meta+K' },
  { id: 'goToFile', label: '跳转到文件', default: 'Meta+P' },
  { id: 'openPreferences', label: '打开偏好', default: 'Meta+,' },
  { id: 'newScriptFile', label: '新建剧本文件', default: 'Meta+N' },
  { id: 'newProject', label: '新建项目', default: 'Meta+Shift+N' },
  { id: 'openProject', label: '打开项目', default: 'Meta+O' },
  { id: 'commit', label: 'Git 提交', default: 'Meta+Shift+C', requiresProject: true },
  { id: 'export', label: '导出', default: 'Meta+E', requiresProject: true },
  { id: 'toggleLeftPanel', label: '切换左面板', default: 'Meta+1' },
  { id: 'showGit', label: '显示 Git 面板', default: 'Meta+2' },
  { id: 'showOutline', label: '显示大纲面板', default: 'Meta+3' },
  { id: 'showCharacter', label: '显示角色面板', default: 'Meta+4' },
  { id: 'showAi', label: '显示 AI 面板', default: 'Meta+5' },
  { id: 'undo', label: '撤销', default: 'Meta+Z' },
  { id: 'redo', label: '重做', default: 'Meta+Shift+Z' }
]
...
/**
 * 计算某命令的有效 accelerator:用户覆盖优先,否则默认;两者皆空返回 null。
